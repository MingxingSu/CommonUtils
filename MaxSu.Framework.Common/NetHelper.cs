...
using MaxSu.Framework.Common.Smtp;
...
namespace MaxSu.Framework.Common
{
    ...
    public class NetHelper
    {
        ...
        {
            //将字符串消息转换成字符数组
            byte[] buffer = NumbericConverter.StringToBytes(msg, Encoding.Default);
        ...
        public static string ReceiveMsg(Socket socket)
        {
            ...
            //转换成字符串，并将其返回
            return NumbericConverter.BytesToString(tempBuffer, Encoding.Default);
        }
        ...
        public static bool SendEmail(string receiveEmail, string msgSubject, string msgBody)
        {
            //创建电子邮件对象
            var email = new MailMessage();
            //设置接收人的电子邮件地址
            email.AddRecipients(receiveEmail);
            //设置邮件的标题
            ...
            {
                //发送电子邮件
                smtp.Send(email, "Username", "passowrd");
