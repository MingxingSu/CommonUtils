...
namespace MaxSu.Framework.Common.Mvvm.Progress
{
	public class ProgressTaskViewModel : ProgressViewModel
	{
		private ProgressReporter _pr = new ProgressReporter();

		public ProgressTaskViewModel(string progressText) :base(progressText) {}

		public override void ReportProgress(int x) {
			if (this.ReportsProgress) {
				if (this.ActionProgressUpdate != null)
					_pr.ReportProgress(() => this.ActionProgressUpdate(x));
			}
			else
		...
		//from: http://nitoprograms.blogspot.com/2010/06/reporting-progress-from-tasks.html
