...
namespace MaxSu.Framework.Common
{
    ...
    public class LzwDecoder
    {
        ...
        public byte[] DecodeImageData(int width, int height, Stream stream)
        {
            ...
            int bits = 0;
            //在编码流中数据的保存形式为byte，而实际编码过程中是找实际编码位来存储的，比如当codeSize=5的时候，那么实际上5bit的数据就应该可以表示一个编码，这样取出来的1个字节就富余了3个bit，这3个bit用于和第二个字节的后两个bit进行组合，再次形成编码值，如此类推
            ...
            while (i < pixelCount)
            {
                ...
                if (top == 0)
                {
                    if (bits < codeSize)
                    {
                        ...
                        //获取本次要处理的数据的值
                        data += buffer[bi] << bits;
                        //此处为何要移位呢，比如第一次处理了1个字节为176，第一次只要处理5bit就够了，剩下3bit留给下个字节进行组合。也就是第二个字节的后两位+第一个字节的前三位组成第二次输出值
