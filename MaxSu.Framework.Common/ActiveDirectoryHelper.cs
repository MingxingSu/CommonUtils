...
{
    internal sealed class ActiveDirectoryHelper
    {
        #region 私有变量
        ...
        public ActiveDirectoryHelper(string sADPath, string sDomainName, string sADUser, string sADUserPWD)
        {
            ADPath = sADPath;
        ...
        {
            var entry = new DirectoryEntry(ADPath + domainReference, ADUser, ADPassword,
        ...
        {
            var entry = new DirectoryEntry(ADPath + domainReference,
            ...
            )
        {
            ...
            try
            {
                ...
                user.Properties["userAccountControl"].Value = //0x200; //ADS_UF_NORMAL_ACCOUNT
                    ActiveDirectoryUserFlag.ADS_UF_NORMAL_ACCOUNT | ActiveDirectoryUserFlag.ADS_UF_DONT_EXPIRE_PASSWD;
                user.CommitChanges();
                if (CreateMail)
                {
        ...
        public static void DisableUser(DirectoryEntry de)
        {
            ...
            de.Properties["userAccountControl"][0] =
                ActiveDirectoryUserFlag.ADS_UF_NORMAL_ACCOUNT | ActiveDirectoryUserFlag.ADS_UF_DONT_EXPIRE_PASSWD |
                ActiveDirectoryUserFlag.ADS_UF_ACCOUNTDISABLE;
            de.CommitChanges();
        ...
        {
            var u = new DirectoryEntry(user_path, ADUser, ADPassword);
            var t = new DirectoryEntry(target_path, ADUser, ADPassword);
            u.MoveTo(t);
            return u.Path;
        }
        ...
        public static bool RenameOU(string oldOUName, string newOUName)
        {
            try
            {
                ...
                {
                    var userEntry = new DirectoryEntry(GetOrganizeNamePath(oldOUName), ADUser, ADPassword);
        ...
        public static bool IsAccountActive(int userAccountControl)
        {
            int userAccountControl_Disabled =
                Convert.ToInt32(ActiveDirectoryUserFlag.ADS_UF_ACCOUNTDISABLE);
        ...
        public static void DetTree(string path)
        {
            try
            {
                var ent = new DirectoryEntry(path, ADUser, ADPassword);
                ent.DeleteTree();
            }
            catch (Exception e)
            {
        ...
        public static string SplitOrganizeNameToDN(string organizeName)
        {
            ...
            {
                string[] allOu = organizeName.Split(new[] {'/', '\\'});
