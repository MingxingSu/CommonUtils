...
namespace MaxSu.Framework.Common
{
    public class ImageHelper
    {
        ...
        #region public static 将图片更改大小后保存成新图片
        /// <summary>
        /// 将图片更改大小后保存成新图片
        /// </summary>
        /// <param name="OldImagePath">原图片路径</param>
        /// <param name="NewImagePath">新图片路径</param>
        /// <param name="NewMaxWidth">新图片最大宽度</param>
        /// <param name="NewMaxHeight">新图片最大高度</param>
        /// <param name="ErrMessage">返回错误信息</param>
        /// <returns>是否成功</returns>
        public static bool ResizeImage(string OldImagePath, string NewImagePath, int NewMaxWidth, int NewMaxHeight)
        {
            try
            {
                Image imgOld, imgNew;
                decimal decOldWidth, decOldHeight, decNewWidth, decNewHeight;

                //加载图片
                imgOld = Image.FromFile(OldImagePath);

                //取得原图宽
                decOldWidth = imgOld.Width;
                //取得原图高
                decOldHeight = imgOld.Height;

                if ((decOldWidth / NewMaxWidth) >= (decOldHeight / NewMaxHeight))
                {
                    //取得缩图宽
                    decNewWidth = NewMaxWidth;
                    //取得缩图高
                    decNewHeight = decOldHeight * decNewWidth / decOldWidth;
                }
                else
                {
                    decNewHeight = NewMaxHeight;
                    decNewWidth = decOldWidth * decNewHeight / decOldHeight;
                }

                //Image.GetThumbnailImageAbort myCallback =new Image.GetThumbnailImageAbort(ThumbnailCallback);//声明回调
                imgNew = imgOld.GetThumbnailImage(System.Convert.ToInt32(decNewWidth), System.Convert.ToInt32(decNewHeight), null, System.IntPtr.Zero);

                imgOld.Dispose();
                //保存缩图
                imgNew.Save(NewImagePath, System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        #region public static 取得图片的宽度
        /// <summary>
        /// 取得图片的宽度
        /// </summary>
        /// <param name="ImagePath">图片的路径</param>
        /// <returns>图片的宽度</returns>
        public static int GetImageWidth(string ImagePath)
        {
            ...
            {
                //加载图片
                imgGet = Image.FromFile(ImagePath);

                //取得原图宽
                intImageWidth = imgGet.Width;
                imgGet.Dispose();
        ...
        #region public static 取得图片的高度
        /// <summary>
        /// 取得图片的高度
        /// </summary>
        /// <param name="ImagePath">图片的路径</param>
        /// <returns>图片的高度</returns>
        ...
        #region public static 比较图片宽度高度
        /// <summary>
        /// 比较图片宽度高度
        /// </summary>
        /// <param name="ImagePath">图片的路径</param>
        /// <returns>比较结果</returns>
        public static int CompareWidthHeight(string ImagePath)
        {
            Image imgGet = null;
            int intImageHeight = 0;
            int intImageWidth = 0;
