...
namespace MaxSu.Framework.Common.Xml
{
    ...
    public class XmlHelper
    {
        ...
        private static XmlDocument XMLLoad(string strPath)
        {
            ...
            try
            {
                string filename = AppDomain.CurrentDomain.BaseDirectory + strPath;
                if (File.Exists(filename)) xmldoc.Load(filename);
            }
            catch (Exception e)
            {
        ...
        public static void CopyNodeList(XmlNodeList objNodeList, XmlNode objDestinationNode, bool blnDeep = true)
        {
            try
            {
                ...
                try
                {
                    enumerator = objNodeList.GetEnumerator() as IEnumerator<XmlNode>;
                    while (enumerator.MoveNext())
                    {
                        var current = (XmlNode)enumerator.Current;
                        XmlNode newChild = objDestinationNode.OwnerDocument.ImportNode(current, blnDeep);
                        objDestinationNode.AppendChild(newChild);
                    }
                }
                finally
                {
        ...
        public static void CreateAttribute(ref XmlNode objXMLNode, string strNewAtttributeName, string strValue)
        {
            try
            {
                XmlAttribute node = objXMLNode.OwnerDocument.CreateAttribute(strNewAtttributeName);
                node.Value = strValue;
                objXMLNode.Attributes.Append(node);
            }
            catch (Exception exception1)
            {
                ProjectData.SetProjectError(exception1);
        ...
        public static string Dataset2XML(DataSet dsData, bool blnReadOnly = false)
        {
            string str;
            try
            {
                ...
                var writer = new XmlTextWriter(w, Encoding.Unicode);
                if (!blnReadOnly)
                {
                    dsData.WriteXml(writer, XmlWriteMode.WriteSchema);
                }
                else
                {
                    dsData.WriteXml(writer);
                }
                writer.Flush();
                w.Position = 0L;
                str = new StreamReader(w).ReadToEnd();
            }
            catch (Exception exception1)
            {
                ProjectData.SetProjectError(exception1);
        ...
        public static string EscapeXML(string str)
        {
            ...
            try
            {
                str2 = SecurityElement.Escape(str);
            }
            catch (Exception exception1)
            {
                ProjectData.SetProjectError(exception1);
        ...
        public static bool HasAttribute(XmlNode xmlNode, string strName)
        {
            return ((xmlNode.Attributes[strName] != null) && (xmlNode.Attributes[strName].Value != ""));
        }
