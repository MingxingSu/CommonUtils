...
namespace MaxSu.Framework.Common.IO
{
    ...
    public class UpLoadFiles : Page
    {
        public string UploadFile(string filePath, int maxSize, string[] fileType, HtmlInputFile TargetFile)
        {
            ...
            var myInfo = new FileInfo(strFileName);
            ...
            if (TargetFile.PostedFile.ContentLength <= MaxSize)
            {
                ...
                {
                    if (strNewName.ToLower() == fileType[i])
                    {
                        typeFlag = true;
                        break;
                    }
        ...
        /// <returns></returns>
        public string UploadFile(string filePath, int maxSize, string[] fileType, HtmlInputFile TargetFile,
                                 out string saveFileName, out int fileSize)
        {
            saveFileName = "";
            fileSize = 0;
            ...
            if (TargetFile.PostedFile.ContentLength <= MaxSize)
            {
                ...
                saveFileName = strFilePath.Substring(strFilePath.IndexOf("FileTransfer\\"));
                TargetFile.PostedFile.SaveAs(strFilePath);
                TargetFile.Dispose();
            }
            else
        ...
        public string UploadFile(string filePath, int maxSize, string[] fileType, string filename,
        ...
        public string GetUploadFileName()
        {
            ...
            Result += time.Year.ToString() + FormatNum(time.Month.ToString(), 2) + FormatNum(time.Day.ToString(), 2) +
                      FormatNum(time.Hour.ToString(), 2) + FormatNum(time.Minute.ToString(), 2) +
                      FormatNum(time.Second.ToString(), 2) + FormatNum(time.Millisecond.ToString(), 3);
            return (Result);
        }
